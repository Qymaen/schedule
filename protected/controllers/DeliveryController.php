...
class DeliveryController extends Controller
{
	public function actionIndex()
	{
		...
		$users = $user->getUsers($params);
		
		// if form submited
		if ((!empty($_POST) and !empty($_POST['group_id']) and !empty($_POST['message']))
				or
				(!empty($_POST) and !empty($_POST['user_id']) and !empty($_POST['message']))) {
			...
			if (!empty($_POST['user_id'])) $params['user_id'] = (int) $_POST['user_id'];
			
			$students = $user->getUsers($params);
